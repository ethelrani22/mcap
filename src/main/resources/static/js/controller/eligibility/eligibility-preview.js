...
(function () {
  ...
  const EligibilityPreview = {
   	updateStudentPreview: function () {
	    ...
	    const qualRequirements = S.qualificationRequirements || [];
	    ...
	    if (!qualRequirements.length) {
	        p.append("No qualification gate configured — any qualification is accepted.");
	    } else {
	        p.append("You must have passed ");
	        qualRequirements.forEach((req, i) => {
	            const s1 = document.createElement("strong");
	            s1.textContent = req.qualificationName || "[Qualification]";
	            p.append(s1);
	            if (req.minPercentage !== null && req.minPercentage !== undefined && req.minPercentage !== "") {
	                const s2 = document.createElement("strong");
	                s2.textContent = `${req.minPercentage}%`;
	                p.append(" with a minimum of ", s2, " overall");
	            }
	            if (i < qualRequirements.length - 1) p.append(", OR ");
	        });
	        p.append(".");
	    }
