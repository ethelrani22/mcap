const EligibilityConfig = (() => {
  ...
  let routes = []; // [ { subjectNames, minScore, useOverall } ]
...
function renderWindows(windows) {
    ...
    ruleSets.forEach((rs) => {
      ...
      reqs.forEach((req) => {
        ...
        if (src === "CUET") {
          ...
          const isOverall = subjectsList.length === 1 && subjectsList[0] === "__OVERALL__";
          routes.push({
            subjectNames: isOverall ? "__OVERALL__" : subjectNamesStr,
            minScore: req.minScore ?? 0,
            useOverall: isOverall
          });
        }
      });
    });
  ...
  function nonCuetMeritLabel(optionIndex) {
    ...
    // Handle __OVERALL__ display
    if (req?.useOverall || req?.subjectNames === "__OVERALL__") {
      return `Rule ${optionIndex + 1} (NON-CUET): Overall Class XII % (Min: ${minTxt}%)`;
    }
  ...
  function normalizeRoutesToSingleRulePerCondition() {
    ...
    routes.forEach((x) => {
      if (x && !Array.isArray(x) && typeof x === "object") {
        const isOverall = x.useOverall === true || x.subjectNames === "__OVERALL__";
        out.push({
          subjectNames: isOverall ? "__OVERALL__" : (x.subjectNames || ""),
          minScore: x.minScore != null ? x.minScore : (x.minPercentage != null ? x.minPercentage : 0),
          useOverall: isOverall
        });
        return;
      }
      if (Array.isArray(x)) {
        ...
        if (first && typeof first === "object") {
          const isOverall = first.useOverall === true || first.subjectNames === "__OVERALL__";
          out.push({
            subjectNames: isOverall ? "__OVERALL__" : (first.subjectNames || ""),
            minScore: first.minScore != null ? first.minScore : (first.minPercentage != null ? first.minPercentage : 0),
            useOverall: isOverall
          });
        }
      }
  ...
  function addCondition() {
    routes.push({ subjectNames: "", minScore: 0, useOverall: false });
    renderRoutes();
    renderNonCuetMeritSourceButtons();
  }
  ...
  function renderRoutes() {
	...
	els.routesContainer.innerHTML = ""; // safe clear
		routes.forEach((req, cIdx) => {
		    const isOverall = !!req.useOverall;
		    const subjects = (!isOverall && req.subjectNames)
		        ? req.subjectNames.split(",").map(s => s.trim()).filter(Boolean)
		        : [];

		    const modeLabel = isOverall
		        ? "Overall Class XII percentage"
		        : (subjects.length > 1 ? "Average of selected subjects" : "Single subject minimum required");

		    const minLabel = isOverall
		        ? "Minimum Overall % Required"
		        : (subjects.length > 1 ? "Minimum Average % Required" : "Minimum % Required");
		    ...
		    toggleDiv.className = "form-check mb-2";

		    const toggleInput = document.createElement("input");
		    toggleInput.type = "checkbox";
		    toggleInput.className = "form-check-input";
		    toggleInput.id = `overall-toggle-${cIdx}`;
		    toggleInput.checked = isOverall;
		    toggleInput.addEventListener("change", () => {
		        routes[cIdx].useOverall = toggleInput.checked;
		        routes[cIdx].subjectNames = toggleInput.checked ? "__OVERALL__" : "";
		        renderRoutes();
		        renderNonCuetMeritSourceButtons();
		        renderNonCuetMeritRuleSets();
		    });
		    ...
		    toggleLabel.className = "form-check-label x-small";
		    toggleLabel.htmlFor = `overall-toggle-${cIdx}`;
		    toggleLabel.textContent = "Use overall Class XII % (instead of specific subjects)";

		    toggleDiv.append(toggleInput, toggleLabel);
		    ...
		    if (!isOverall) {
		        ...
		        note.className = "x-small text-muted fst-italic p-2 bg-light rounded border";
		        note.textContent = "Will compute: total marks obtained ÷ total max marks across all Class XII subjects × 100";
		    ...
		    card.append(header, toggleDiv, row);
  ...
  function saveConfiguration() {
    ...
    // Read selected qualification IDs from the Choices multi-select
    const acceptedQualificationIds = choicesInstances['baseQualification']
        ? choicesInstances['baseQualification'].getValue(true).map(v => parseInt(v, 10)).filter(Boolean)
        : [];

    const payload = {
      admissionCode: currentWindowCode,
      programmeId: currentProgrammeId,
      acceptedQualificationIds,
    ...
    if (!acceptedQualificationIds.length) { return showToast("Please select at least one accepted qualification", "warning"); }
      ...
      if (routes.length) {
        routes.forEach((req, idx) => {
          ...
          if (!isOverall && !subjects.length) {
            throw new Error(`NON-CUET Rule ${idx + 1}: Please select at least 1 subject or enable overall % mode.`);
          }
          ...
          payload.ruleSets.push({
            ...
            subjectRequirements: [{ subjectNames: subjects, minScore: minPct, calculationType: (!isOverall && subjects.length > 1) ? "AGGREGATE_AVERAGE" : "INDIVIDUAL_SUBJECT", scoreSource: "NON_CUET" }]
          });
        });
      }
    } catch (e) {
