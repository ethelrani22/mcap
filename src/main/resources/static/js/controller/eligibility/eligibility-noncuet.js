...
(function () {
  ...
  function toggleNonCuet(on) {
    ...
    // FIX: Wire the "Add Rule" button for the Direct Admission route.
    // Previously this listener was never attached, causing the button to do
    // nothing on click while the equivalent CUET button worked fine.
    const addBtn = document.getElementById("btnAddNonCuet");
    if (addBtn && !addBtn._bound) {
        addBtn.addEventListener("click", () => EligibilityConfig.addCondition());
        addBtn._bound = true;
    }
  ...
  function renderRoutes() {
	...
	(S.routes || []).forEach((reqs, cIdx) => {
	    ...
	    const isOverall = (req.subjectNames || "").trim() === "__OVERALL__";
	    ...
	    overallWrap.className = "form-check form-switch mb-2";

	    const overallToggle = document.createElement("input");
	    overallToggle.type = "checkbox";
	    overallToggle.className = "form-check-input";
	    overallToggle.id = `overall-pct-${cIdx}`;
	    overallToggle.checked = isOverall;

	    const overallLabel = document.createElement("label");
	    overallLabel.className = "form-check-label small fw-bold";
	    overallLabel.setAttribute("for", overallToggle.id);
	    overallLabel.textContent = "Use overall Class XII % (no specific subjects required)";

	    overallWrap.append(overallToggle, overallLabel);

	    // ===== SUBJECT PICKER WRAPPER (hidden when overall toggle is on) =====
	    const subjectWrapper = document.createElement("div");
	    subjectWrapper.id = `subject-wrapper-${cIdx}`;
	    if (isOverall) subjectWrapper.classList.add("d-none");
	    ...
	    subjectWrapper.append(label1, select);

	    overallToggle.addEventListener("change", (e) => {
	        if (e.target.checked) {
	            S.routes[cIdx][0].subjectNames = "__OVERALL__";
	            subjectWrapper.classList.add("d-none");
	        } else {
	            S.routes[cIdx][0].subjectNames = "";
	            subjectWrapper.classList.remove("d-none");
	        }
	        if (window.EligibilityPreview) EligibilityPreview.updateStudentPreview();
	    });

	    col1.append(overallWrap, subjectWrapper);
      ...
      S.routes.forEach((reqs, cIdx) => {
          const req = reqs[0];
          // Skip Choices init for __OVERALL__ rules — no subject picker needed
          if ((req.subjectNames || "").trim() === "__OVERALL__") return;
