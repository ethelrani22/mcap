...
(function () {
  ...
  const S = (window.EligibilityConfigState = window.EligibilityConfigState || {
    ...
    // Each entry: { qualificationId, qualificationName, minPercentage }
    ...
    tiebreakerOptions: [
        // NOTE: these `value`s must match the field strings TieBreakerService
        // switches on exactly -- they previously used different casing
        // (CLASS_12_MARKS etc.) which never matched anything on the backend,
        // so every tie-breaker rule was silently a no-op. Fixed to match.
        { value: "class12Percentage", label: "Class XII Percentage" },
        { value: "entranceExamPercentile", label: "Entrance Exam Score" },
        { value: "ugDegreePercentage", label: "UG Degree Percentage" },
        { value: "communityCategory", label: "Community Category Priority" },
        { value: "dateOfBirth", label: "Date of Birth (Older first)" },
        { value: "currentYearPassout", label: "Preference to Current-Year Pass-outs" },
        { value: "subjectMarks", label: "Specific Subject Marks", needsSubject: true }
    ],
  ...
  function cacheEls() {
    S.els = {
      ...
      addQualificationRequirementBtn: document.getElementById("addQualificationRequirementBtn"),
      qualificationRequirementRows: document.getElementById("qualificationRequirementRows"),
      qualificationRequirementEmptyHint: document.getElementById("qualificationRequirementEmptyHint"),
  ...
  function renderQualificationDropdown() {
    ...
    const options = sortedQuals.map(q => ({ value: String(q.id), label: q.name }));
    ...
    S.choicesInstances['baseQualification'] = new Choices(els.baseQualificationId, {
        ...
        searchEnabled: true,
        placeholderValue: 'Select a qualification to add...',
        itemSelectText: '',
        shouldSort: false
    });
  ...
  function findQualificationById(id) {
    return (S.masterData.qualifications || []).find(q => String(q.id) === String(id));
  }

  function addQualificationRequirement() {
    const { els } = S;
    const choices = S.choicesInstances['baseQualification'];
    if (!choices) return;

    const selectedValue = choices.getValue(true); // single-select → string id or ""
    if (!selectedValue) {
      showToast("Select a qualification first.", "warning");
      return;
    }

    if (S.qualificationRequirements.some(q => String(q.qualificationId) === String(selectedValue))) {
      showToast("That qualification has already been added.", "warning");
      return;
    }

    const qual = findQualificationById(selectedValue);
    S.qualificationRequirements.push({
      qualificationId: selectedValue,
      qualificationName: qual ? qual.name : selectedValue,
      minPercentage: null
    });

    renderQualificationRequirementRows();
    if (window.EligibilityPreview) EligibilityPreview.updateStudentPreview();
  }

  function renderQualificationRequirementRows() {
    const { els } = S;
    if (!els.qualificationRequirementRows) return;

    els.qualificationRequirementRows.innerHTML = "";

    if (els.qualificationRequirementEmptyHint) {
      els.qualificationRequirementEmptyHint.style.display =
        S.qualificationRequirements.length ? "none" : "block";
    }

    S.qualificationRequirements.forEach((req, index) => {
      const row = document.createElement("div");
      row.className = "d-flex align-items-center gap-2 p-2 border rounded";
      row.dataset.qualificationId = req.qualificationId;

      const nameSpan = document.createElement("span");
      nameSpan.className = "flex-grow-1 fw-semibold small qual-name";
      nameSpan.textContent = req.qualificationName;
      ...
      pctInput.className = "form-control qual-min-pct";
      pctInput.placeholder = "Min %";
      pctInput.min = "0";
      pctInput.max = "100";
      pctInput.step = "0.01";
      pctInput.value = req.minPercentage ?? "";
      pctInput.addEventListener("input", () => {
        const val = pctInput.value === "" ? null : parseFloat(pctInput.value);
        S.qualificationRequirements[index].minPercentage = val;
        if (window.EligibilityPreview) EligibilityPreview.updateStudentPreview();
      });

      const pctSuffix = document.createElement("span");
      pctSuffix.className = "input-group-text";
      pctSuffix.textContent = "%";

      inputGroup.appendChild(pctInput);
      inputGroup.appendChild(pctSuffix);

      const removeBtn = document.createElement("button");
      removeBtn.type = "button";
      removeBtn.className = "btn btn-sm btn-outline-danger remove-qual-req";
      removeBtn.innerHTML = '<i class="fas fa-times"></i>';
      removeBtn.addEventListener("click", () => {
        S.qualificationRequirements.splice(index, 1);
        renderQualificationRequirementRows();
        if (window.EligibilityPreview) EligibilityPreview.updateStudentPreview();
      });
      ...
      els.qualificationRequirementRows.appendChild(row);
    });
  }
  ...
  function populateForm(data) {
    ...
    // Populate accepted qualifications, each with its own min percentage
    S.qualificationRequirements = Array.isArray(data.qualificationRequirements)
        ? data.qualificationRequirements.map(qr => ({
              qualificationId: String(qr.qualificationId),
              qualificationName: qr.qualificationName,
              minPercentage: qr.minPercentage ?? null
          }))
        : [];
    renderQualificationRequirementRows();
  ...
  function resetForm() {
    const { els } = S;
    if (S.choicesInstances['baseQualification']) {
        S.choicesInstances['baseQualification'].removeActiveItems();
    }
    S.qualificationRequirements = [];
    renderQualificationRequirementRows();
  ...
  function renderTieBreakerSortable() {
      ...
      S.tiebreakerConfig.forEach(tb => {
          ...
          if (opt) sortedOptions.push({ ...opt, enabled: true, subjectId: tb.subjectId });
      });
      ...
      container.replaceChildren();
		(sortedOptions || []).forEach((opt) => {
		    ...
		    item.className = "list-group-item d-flex align-items-center flex-wrap border mb-2 rounded shadow-sm bg-white";
		    ...
		    // ===== Subject dropdown (only for "subjectMarks") =====
		    if (opt.needsSubject) {
		        const select = document.createElement("select");
		        select.className = "form-select form-select-sm tb-subject-select mt-2 ms-5 w-auto";
		        select.style.display = opt.enabled ? "inline-block" : "none";
		        ...
		        (S.masterData.subjects || []).forEach(sub => {
		            const o = document.createElement("option");
		            o.value = sub.subjectId;
		            o.textContent = sub.subjectName;
		            if (opt.subjectId != null && String(opt.subjectId) === String(sub.subjectId)) {
		                o.selected = true;
		            }
		        ...
		        input.addEventListener("change", () => {
		            select.style.display = input.checked ? "inline-block" : "none";
		        });
  ...
  function applyTieBreakerConfig() {
      ...
      items.forEach(item => {
          ...
          if (isChecked) {
              const entry = { field: value, priority: priority++ };
              const subjectSelect = item.querySelector(".tb-subject-select");
              if (subjectSelect) {
                  if (!subjectSelect.value) {
                      alert("Please select a subject for the 'Specific Subject Marks' tie-breaker, or turn it off.");
                      priority--; // rollback -- don't count this entry
                      return;
                  }
                  entry.subjectId = parseInt(subjectSelect.value, 10);
              }
  ...
  // Add qualification requirement button
  S.els.addQualificationRequirementBtn?.addEventListener("click", () => {
    if (typeof EligibilityConfig.addQualificationRequirement === "function") {
      EligibilityConfig.addQualificationRequirement();
    }
  });
  ...
  Object.assign(window.EligibilityConfig, {
    ...
    addQualificationRequirement, renderQualificationRequirementRows
  });
