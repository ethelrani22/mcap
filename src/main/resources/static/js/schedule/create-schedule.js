...
async function renderWindowCards() {
    ...
    for (const windowData of allWindowsData) {
        ...
        const statusBadgeMap = {
            UPCOMING: { cls: 'badge-upcoming', label: 'Upcoming Window' },
            ACTIVE: { cls: 'badge-active', label: 'Active Window' },
            CLOSED: { cls: 'badge-closed', label: 'Closed Window' }
        };
        const statusBadge = statusBadgeMap[windowData.windowStatus] || statusBadgeMap.UPCOMING;

        card.innerHTML = `
            <div class="window-card shadow-sm" data-admission-code="${windowData.admissionCode}">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <span class="window-badge ${statusBadge.cls}">${statusBadge.label}</span>
                    ${isComplete
...
function applyDateConstraints(activeTemplate) {
    ...
    if (activeTemplate.category === 'PRE_ADMISSION') {
        ...
        minStart = selectedWindowData.originalEndDate || selectedWindowData.endDate;
        const phaseLabel = activeTemplate.category === 'CORRECTION' ? 'Correction' : 'Counselling';
        ruleText += `<li><strong>${phaseLabel} Limit:</strong> Cannot begin until <strong>after</strong> the Student Window's original close date on <span class="badge bg-white text-dark border border-secondary">${formatDateTime(minStart)}</span>.</li>`;
    }
