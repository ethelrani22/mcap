...
export function initializeSeatAllotmentPage() {
    ...
    if (rejectBtn) rejectBtn.onclick = handleRejectionPrompt;

    const confirmRejectBtn = document.getElementById('confirmRejectBtn');
    if (confirmRejectBtn) confirmRejectBtn.onclick = handleRejection;

    const rejectReasonSelect = document.getElementById('rejectReasonSelect');
    const rejectOtherReasonWrapper = document.getElementById('rejectOtherReasonWrapper');
    const rejectOtherReason = document.getElementById('rejectOtherReason');

    const updateRejectConfirmState = () => {
        if (!rejectReasonSelect || !confirmRejectBtn) return;
        const selected = rejectReasonSelect.value;
        const isOther = selected === 'OTHER';
        if (rejectOtherReasonWrapper) rejectOtherReasonWrapper.style.display = isOther ? '' : 'none';

        const otherTextFilled = !isOther || (rejectOtherReason?.value || '').trim().length > 0;
        confirmRejectBtn.disabled = !selected || (isOther && !otherTextFilled);
    };

    if (rejectReasonSelect) rejectReasonSelect.addEventListener('change', updateRejectConfirmState);
    if (rejectOtherReason) rejectOtherReason.addEventListener('input', updateRejectConfirmState);
...
function handleRejectionPrompt() {
    ...
    const otherWrapper = document.getElementById('rejectOtherReasonWrapper');
    const otherText = document.getElementById('rejectOtherReason');
...
async function handleRejection() {
    ...
    if (!selectedValue) return showError('Please select a reason for rejection.');

    let reason;
    if (selectedValue === 'OTHER') {
        reason = (document.getElementById('rejectOtherReason')?.value || '').trim();
        if (!reason) return showError('Please specify your reason.');
    } else {
        // Use the select's visible label (e.g. "Got admission elsewhere") as the stored reason
        reason = select.options[select.selectedIndex]?.text?.trim() || selectedValue;
    }

    showLoading("Processing rejection...");
    try {
        await axios.post('/api/applicants/counseling/reject', { allotmentId, reason });
        await showSuccess("Offer Rejected. You may be considered for other preferences in the next round.");
    ...
    try {
        ...
        if (rounds && rounds.length > 0) {
            ...
            rounds.forEach(round => {
                ...
                // ACCEPTED now ALWAYS means the admission fee has already been paid
                // (status only ever flips PENDING -> ACCEPTED on confirmed payment), so
                // there is never a "Pay Admission Fee" step left to show for it.
                // For SLIDE_UP, only show the payment link if the flat slide-up fee
                // genuinely hasn't been paid yet — otherwise this becomes the confusing
                // "pay again" prompt after a successful payment.
                if (round.status === 'SLIDE_UP' && !round.slideFeePaid && round.allotmentId) {
...
window.showSeatFeeModal = function (allotmentId, programmeOfferedId, isSlideUp) {
    ...
    fetch('/applicants/payment/seat-fee-structure/' + encodeURIComponent(programmeOfferedId) + '?isSlideUp=' + isSlideUp, {
